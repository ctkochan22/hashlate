...
- Create #hashify method that will create a hash where keys are a string path to element and the value pair will be the element as a string.
  **Method Psuedocode
  ...
  - Check if element is a hash
    - if true...
      - Check to see how many keys are within the hash.
      - If only one, run #hashify on element
      - If more than one, iterate through and assign key value pairs for each key
      - Then run #hashify on element
- Return hash
